...
export const LocaleSelector: FC<LocaleSelectorProps> = ({ currentLocale, locales, onSelectLocale }) => {
  ...
  const regionsOptions = regions.map(item => ({
    label: `${item.localeEmoji} ${COUNTRY_NAMES?.[item.region] || item.region}`,
    value: item.region,
  }));
  ...
  return (
    <div className="m-4 mb-6">
      <p className="font-normal text-secondary mt-2 mb-2">Country</p>
      <Dropdown
        title="Country"
      ...
      />
      <p className="font-normal text-secondary mt-8 mb-2">Language</p>
      <Dropdown
